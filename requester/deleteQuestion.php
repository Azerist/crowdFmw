...
if(isset($_POST['sure']) && $_POST['sure'] == 'yes'){
	...
	$query = $db->query('SELECT task.id_requester FROM task,question WHERE question.id_task=task.id AND question.id='.$_GET['id']) or dbErr();

	$result = $query->fetch_assoc();
	if($result == NULL || $result['id_requester'] != $_SESSION['userid']){
		error('Error : You are not owner of this question !');
	}

	//Delete the input file linked to the question
	$query = $db->query('SELECT input FROM question WHERE id='.$_GET['id']) or dbErr();
	...
	$query = $db->query('DELETE FROM question WHERE id='.$_GET['id']) or dbErr();
...
$query = $db->query('SELECT count(*) FROM contribution WHERE id_question='.$_GET['id']) or dbErr();
