...
$query = $db->query("SELECT id FROM question WHERE id_task=$_GET[task]") or dbErr();

if($query->num_rows == 0)
	error("The task id provided is incorrect.");
...
if(!isset($_POST["ans-$question[id]"]))
	error("The data sent by your browser is incorrect or incomplete.");
...
//execute the query
$db->query($sql) or dbErr();

//remove the attribution if existing
$db->query("DELETE FROM attribution WHERE id_question=$question AND id_user=$userid") or dbErr();
